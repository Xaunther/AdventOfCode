...
size_t Part1( const std::string& aFileName )
{
	const auto& caverns = CreateFromFile<CCaverns>( aFileName );
	const CCavernPaths cavernPaths{ caverns.GetCavernMap() };
	return cavernPaths.GetPaths().size();
}

size_t Part2( const std::string& aFileName )
{
	const auto& caverns = CreateFromFile<CCaverns>( aFileName );
	const CCavernPaths cavernPaths{ caverns.GetCavernMap(), false };
	return cavernPaths.GetPaths().size();
}
